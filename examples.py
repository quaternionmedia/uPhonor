...
import time
import threading
from uphonor_python import UPhonor, uphonor_session, HoloState, PlaybackMode, ConfigResult
...
def basic_usage_example():
    ...
    with uphonor_session() as uphonor:
        ...
        if loop:
            print(f"Loop for note 60 - State: {loop.current_state}, Ready: {loop.loop_ready}")
...
def loop_management_example():
    ...
    with uphonor_session() as uphonor:
        ...
        for note, loop in all_loops.items():
            print(f"  Note {note}: {loop.recorded_frames} frames, "
...
def configuration_example():
    ...
    with uphonor_session() as uphonor:
        ...
        if result == ConfigResult.SUCCESS:
            ...
            print(f"Failed to save configuration: {UPhonor.get_config_error_message(result)}")
        
        # Reset to defaults
        uphonor.reset_to_defaults()
        print(f"After reset - Volume: {uphonor.get_volume()}, "
        ...
        if result == ConfigResult.SUCCESS:
            ...
            print(f"After load - Volume: {uphonor.get_volume()}, "
                  f"Speed: {uphonor.get_playback_speed()}, "
                  f"Pitch: {uphonor.get_pitch_shift()}")
        else:
            print(f"Failed to load configuration: {UPhonor.get_config_error_message(result)}")
        ...
        if validation_result == ConfigResult.SUCCESS:
            ...
            print(f"Configuration file validation failed: {UPhonor.get_config_error_message(validation_result)}")
