...
class HoloPhonoIntegration:
    ...
    def handle_midi_note_on(self, channel: int, note: int, velocity: int):
        ...
        if self.uphonor:
            ...
            if loop:
                print(f"🎵 Note {note}: {loop.current_state.name}, "
    ...
    def print_status(self):
        ...
        if all_loops:
            ...
            for note, loop in all_loops.items():
                print(f"  Note {note:3d}: {loop.current_state.name:10s} "
    ...
    def save_session(self, filename: str = None):
        ...
        if self.uphonor:
            ...
            if result.value == 0:  # SUCCESS
                ...
                print(f"❌ Failed to save session: {UPhonor.get_config_error_message(result)}")
    ...
    def load_session(self, filename: str):
        ...
        if self.uphonor:
            ...
            if result.value == 0:  # SUCCESS
                ...
                print(f"❌ Failed to load session: {UPhonor.get_config_error_message(result)}")
